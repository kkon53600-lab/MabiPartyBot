...
async def send_reminder(message, target_time, remind_time):
    try:
        ...
        if participants:
            ...
            await message.channel.send(f"🔔 파티 출발 15분 전입니다! (참여 이모지 반응이 없습니다.)")
...
async def on_message(message):
    ...
    if message.content.startswith('!알림'):
        try:
            ...
            if target_time < datetime.now(): target_time = target_time.replace(year=datetime.now().year + 1)
            remind_time = target_time - timedelta(minutes=15)
            if remind_time < datetime.now():
                await message.channel.send("⚠️ 알림 시간(출발 15분 전)이 이미 지났습니다!")
                return
            if message.channel.id in scheduled_tasks: scheduled_tasks[message.channel.id].cancel()
...
async def on_raw_reaction_add(payload):
    ...
    if payload.emoji.name in TARGET_EMOJIS:
        ...
        for reaction in message.reactions:
            if reaction.emoji.name in TARGET_EMOJIS:
                async for user in reaction.users():
                    if not user.bot: participants_list.append(user)
